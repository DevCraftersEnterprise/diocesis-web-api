...
| BUG-DJANGO-012 | ARCHITECTURE | vistas de detalle/edicion de varios modulos | Soft-delete incoherente entre modulos. | **POLITICA ADOPTADA (FASE 3)**: `GET /{id}/` NUNCA filtra `isActive` (devuelve la fila aunque este borrada); `PUT`/`DELETE` exigen fila activa (404 si borrada, hay que `habilitar` antes); `DELETE` fija `isActive=false`+`deletedAt`+`deletedBy`; `habilitar` limpia `deletedAt`/`deletedBy`. Documentado en `contract-matrix.md`. Aplicado a decanatos (3.1). |
| BUG-DJANGO-022 | BUG | `apps/decanatos/views.py` / `apps/colonias/views.py` GET detail + FE `parish-details.ts` | El detalle filtraba `isActive=True` -> 404 en filas borradas -> rompia `parish-details` (`forkJoin` sin handler). Relacionado con BUG-DJANGO-012. | **RESUELTO en decanatos (Tarea 3.1)**: `GET /decanatos/{id}/` devuelve la fila aunque este soft-deleted. Colonias en 3.2. Ya no hace falta el handler de error en el frontend para este caso. |
