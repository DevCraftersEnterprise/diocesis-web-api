...
| BUG-DJANGO-023 | DATABASE | `apps/carrusel` + BD de produccion | La migracion `carrusel/0003_carrusel_basemodel_fields` **no esta aplicada en prod**: `carrusel_carrusel` solo tiene `id, url, isImage, createdAt, createdBy_id, isActive` (faltan `updatedAt`, `deletedAt`, `updatedBy_id`, `deletedBy_id`). El modelo `Carrusel(BaseModel)` y `CarruselView.put`/`delete` esperan esas columnas -> `save()` contra prod escribiria columnas inexistentes (probable 500; confirmar en runtime). Detectado en Tarea 0.3. Ver `docs/db/notes.md` seccion 1. | **Decidido (ADR-004 DQ3-B):** migracion aditiva `0002-carrusel-basemodel-fields` posterior a la baseline (anade las 4 columnas + FK + indices, backfill de `updatedAt` desde `createdAt` en las 21 filas); la entidad `Carrusel` extiende `BaseEntity` completo. Resuelve el bug. La respuesta de `/carrusel/` gana `updatedAt`/`deletedAt`/`updatedBy`/`deletedBy` (delta aditivo no disruptivo). Fase 5. |
