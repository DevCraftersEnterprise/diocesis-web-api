...
| TEST-001 | TESTING | ambos proyectos | Cobertura de tests nula -> sin red de seguridad de referencia. | **RESUELTO**: tests por slice en cada modulo (unit + e2e contra el oraculo, 179 unit + 114 e2e al cierre de FASE 9) + arnes de paridad (`test/parity/`) completado en la Tarea 9.4 — 22 casos de solo lectura, 0 fallos, auto-chequeo oraculo-vs-oraculo 22/22 OK. El arnes tenia un bug que lo hacia crashear en el primer diff real (`short()` sobre `undefined`) y varios casos sin `ignore`/`expectedDelta` para deltas ya conocidos (`next`/`previous`, BUG-DJANGO-023): nunca se habia corrido de punta a punta hasta esa tarea. |
