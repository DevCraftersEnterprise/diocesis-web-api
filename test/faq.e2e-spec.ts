...
import { PreguntaFrecuente } from './../src/modules/isma/faq/entities/pregunta-frecuente.entity';
import { Usuario } from './../src/modules/users/entities/usuario.entity';
import { seedUser, type SeededUser } from './helpers/seed-user';

const BASE = '/api/isma/preguntas-frecuentes';

describe('ISMA — Preguntas frecuentes (e2e)', () => {
  let app: INestApplication<App>;
  let repo: Repository<PreguntaFrecuente>;
  ...
  const validBody = () => ({
    question: `e2e_pregunta_${randomUUID().slice(0, 6)}`,
    answer: 'Respuesta',
    order: 1,
  });
  const seedPregunta = async (
    over: Partial<PreguntaFrecuente> = {},
  ): Promise<string> => {
    const id = randomUUID();
    await repo.insert({
      id,
      question: `e2e_pregunta_${id.slice(0, 5)}`,
      answer: 'Respuesta',
      order: 1,
      isActive: true,
      createdAt: new Date(),
  ...
  beforeAll(async () => {
    ...
    repo = app.get(getRepositoryToken(PreguntaFrecuente));
  ...
  it('GET / publico -> objeto paginado ordenado por order', async () => {
    await seedPregunta({ order: 2 });
    await seedPregunta({ order: 1 });
  ...
  it('GET /:id soft-deleted -> 200; GET /<no-uuid> -> 404', async () => {
    const id = await seedPregunta({ isActive: false, deletedAt: new Date() });
  ...
  it('PUT /:id edita (updatedBy); PUT sobre borrado -> 404', async () => {
    ...
    const delId = await seedPregunta({ isActive: false });
    await request(app.getHttpServer())
      .put(`${BASE}/${delId}`)
      .set(auth(adminToken))
      .send({ order: 1 })
  ...
  it('POST /habilitar/:id -> 200 texto exacto; sobre activa -> 404', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set(auth(adminToken))
      .expect(200)
      .expect({ detail: 'Pregunta frecuente habilitada correctamente.' });

    const activeId = await seedPregunta();
    await request(app.getHttpServer())
